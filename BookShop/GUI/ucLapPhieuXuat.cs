...
namespace BookShop.GUI
{
    public partial class ucLapPhieuXuat : UserControl
    {
        ...
        private bool Check()
        {
            ...
            int cnt = db.HOADONBANs.Where(p => p.MAHOADON == z).ToList().Count;
            if (cnt > 0)
            {
        ...
        private void LoadDgvChiTietNhap()
        {
            ...
            int i = 0;
            dgvChiTietHoaDonMain.DataSource = db.CHITIETHOADONs.Where(p => p.HOADONBANID == pn.ID).ToList()
                                            .Select(p => new
                                            {
                                                ID = p.ID,
                                                STT = ++i,
